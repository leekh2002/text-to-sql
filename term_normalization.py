...
# 실제 DB 조회 결과를 이 리스트들로 옮겨 적어서 사용한다.
REFERENCE_CATEGORY = [
]
REFERENCE_SUBJECT = [
]
...
def load_reference_values() -> dict[str, list[str]]:
    CATEGORY.clear()
    CATEGORY.extend(REFERENCE_CATEGORY)
    SUBJECT.clear()
    SUBJECT.extend(REFERENCE_SUBJECT)
    DEPARTMENT.clear()
    DEPARTMENT.extend(REFERENCE_DEPARTMENT)
...
def find_best_reference_match(entity_text: str, candidates: list[str]) -> tuple[str | None, float]:
    if not entity_text or not candidates:
        return None, 0.0

    match = process.extractOne(entity_text, candidates, scorer=fuzz.ratio)
    if match is None:
        return None, 0.0
...
def correct_ner_entities(entities: list[dict]) -> list[dict]:
    ...
    for entity in entities:
        ...
        best_match, _ = find_best_reference_match(entity["text"], candidates)
        corrected_entities.append(
            {
                **entity,
